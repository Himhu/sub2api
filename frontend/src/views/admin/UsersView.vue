        ...
        <div class="py-1">
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <!-- Upgrade to Agent (only for non-agents) -->
              <button
                v-if="!user.is_agent"
                @click="handleUpgradeToAgent(user); closeActionMenu()"
                class="flex w-full items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-dark-700"
              >
                <svg class="h-4 w-4 text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
...
const handleUpgradeToAgent = async (user: AdminUser) => {
  ...
  const confirmed = window.confirm(t('admin.users.upgradeToAgentConfirm', { email: user.email }))
  if (!confirmed) return
