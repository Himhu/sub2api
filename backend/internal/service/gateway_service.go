...
type ModelLoadInfo struct {
	CallCount  int64     // 当前分钟调用次数
	LastUsedAt time.Time // 最后调度时间（零值表示未调度过）
}
...
type GatewayCache interface {
	...
	// IncrModelCallCount 递增模型调用计数，用于负载均衡
...
type AvailableModelsResult struct {
	Models []string
	Source string // "mapping", "unlimited", "no_accounts", "all_paused", "error"
}
...
func (s *GatewayService) GetAvailableModelsWithSource(ctx context.Context, groupID *int64, platform string) AvailableModelsResult {
	...
	if err != nil {
		return AvailableModelsResult{Models: nil, Source: "error"}
	}
	if len(accounts) == 0 {
	...
	// Filter to schedulable accounts only
	schedulable := make([]Account, 0, len(accounts))
	for _, acc := range accounts {
		if acc.Schedulable {
	...
	// Collect unique models from schedulable accounts
	modelSet := make(map[string]struct{})
	hasAnyMapping := false

	for _, acc := range schedulable {
