...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	for {
		...
		if account.Platform == service.PlatformAntigravity {
			result, err = h.antigravityGatewayService.ForwardGemini(c.Request.Context(), c, account, modelName, action, stream, body, hasBoundSession)
		} else {
		...
		go func(result *service.ForwardResult, usedAccount *service.Account, ua, ip string, bt int8) {
